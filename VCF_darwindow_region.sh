...
# software:
BCFTOOLS=/home/mdejong/software/bcftools/bcftools-1.20/bcftools	# Path to bcftools executable. Needed for filtering on INDELS only. Alternatively you can use zgrep command (see section 'remove_indels')
TABIX=/opt/software/htslib/htslib-1.9/bin			# NOT to tabix executable itself, but to bin directory which contains tabix and bgzip executable. Without forward slash at the end
...
#MYBED=mybed10.txt
#MYPREFIX=Roe71
#MYPATH=/home/mdejong/roedeer/snpfiles_September2023
#MYVCF=/home/mdejong/roedeer/snpfiles_September2023/Roe71.globalfilter.thin0.mybed20.txt.vcf.gz	# Input file should be compressed using bcftools view -O z (so: NOT with gzip, because this won't be accepted by tabix). 
...
# preparatory steps:
do_index=TRUE						## index input vcf file (output file will have the suffix .tbi)
extract_contiginfo=TRUE				## be aware: set sample_scores to TRUE if intending to run Run of Homozygosity analyses, otherwise the headers of the output files will be incomplete 
extract_samples=TRUE					##

# actual analyses:
run_loop=TRUE						## only set to TRUE after having run the three preparatory steps
...
MYVCF=${MYPATH}/${MYPREFIX}.globalfilter.thin0.${MYBED}.vcf.gz
ls $MYVCF
